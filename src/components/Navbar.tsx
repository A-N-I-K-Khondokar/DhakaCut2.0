    ...
    <nav className="bg-white border-b border-gray-150 sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <div className="hidden md:flex items-center space-x-8">
            <Link 
              to="/" 
              className={`text-sm font-medium transition-colors ${isActive('/') ? 'text-primary border-b-2 border-primary py-5' : 'text-gray-600'}`}
            >
              Home
            </Link>
            <Link 
              to="/salons" 
              className={`text-sm font-medium transition-colors ${isActive('/salons') ? 'text-primary border-b-2 border-primary py-5' : 'text-gray-600'}`}
            >
              Book Salon
            </Link>
            <Link 
              to="/map-search" 
              className={`text-sm font-medium transition-colors ${isActive('/map-search') ? 'text-primary border-b-2 border-primary py-5' : 'text-gray-600'}`}
            >
              📍 Find on Map
            </Link>
      ...
      {isOpen && (
        <div className="md:hidden border-t border-gray-150 bg-white px-2 pt-2 pb-4 space-y-1 animate-slide-in">
          <Link
            to="/"
            className={`block px-3 py-2 rounded text-base font-medium ${isActive('/') ? 'bg-primary-light text-primary' : 'text-gray-700'}`}
          >
            Home
          </Link>
          <Link
            to="/salons"
            className={`block px-3 py-2 rounded text-base font-medium ${isActive('/salons') ? 'bg-primary-light text-primary' : 'text-gray-700'}`}
          >
            Book Salon
          </Link>
          <Link
            to="/map-search"
            className={`block px-3 py-2 rounded text-base font-medium ${isActive('/map-search') ? 'bg-primary-light text-primary' : 'text-gray-700'}`}
          >
            📍 Find on Map
          </Link>
